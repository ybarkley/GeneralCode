...
#Plot
png(filename='C:\\Users\\Yvonne\\Documents\\PHD\\CHP2&3-Sperm\\screenshots\\GPS_1641.png', width=960, height=400)

ggplot(GPS_data, aes(UTC, Latitude)) + 
  theme_bw()+ 
  theme(axis.text.x = element_text(angle=45, hjust=1), plot.margin = margin(10,20,10,10))+
  geom_point()+ geom_line()+
  #scale_y_discrete(breaks = seq(-55, 50, by=10), labels = seq(-55, 50, by=10))+
...
#Parse it out
GPS_data$Year <- year(GPS_data$UTC)
GPS_data$Month <- month(GPS_data$UTC)
GPS_data$Day <- day(GPS_data$UTC)
GPS_data$hour <- hour(GPS_data$UTC)
GPS_data$min <- minute(GPS_data$UTC)
GPS_data$sec <- second(GPS_data$UTC)

#Fix the problem
GPS_data$Month[which(GPS_data$Month != 05 & GPS_data$Day >= 08 & GPS_data$Day <= 31)] = '5' 
#wrongMonth$Month[which(wrongMonth$Month == 06)] = '05'
#Put it back together
GPS_data$UTC = ymd_hms(paste(GPS_data$Year, GPS_data$Month, GPS_data$Day,  GPS_data$hour, GPS_data$min, GPS_data$sec))
#Remove extra rows
GPS_data<- GPS_data[, c(1:10)]
GPS_data<- GPS_data[, -c(3:6)]

write.csv(GPS_data, 'C:\\Users\\Yvonne\\Documents\\PHD\\CHP2&3-Sperm\\data\\All GPS Data\\GPS_1303.raw', row.names = F)
